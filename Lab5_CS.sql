...
select customer_name as name , count(p.order_id) as total_orders, sum(p.amount) as total_payment from customers c
join orders o on o.customer_id = c.customer_id
...
join orders o on o.customer_id = c.customer_id
join payments p on p.order_id = o.order_id
where payment_id = (select payment_id from payments where amount>100);
